...
0. Authenticate once per session: `export CLOUDTOWER_ENDPOINT=<base-url>` then `bash scripts/call.sh login`. **The login command IS the credential check** — it knows every credential variable and the cached token, so run it instead of inspecting `env` yourself, and ask the user only if it errors naming a missing variable
...
`scripts/call.sh` needs `CLOUDTOWER_ENDPOINT` (base URL, e.g. `https://tower.example.com`; paths in operation files already include the `/v2/api` prefix) plus a token. **Credentials are usually already present** — platforms inject `CLOUDTOWER_USERNAME`/`CLOUDTOWER_USER` and `CLOUDTOWER_PASSWORD`, and a token cached in `/tmp/cloudtower.env` is loaded automatically — so set the endpoint, run `bash scripts/call.sh login` once, and ask the user only if it reports a missing variable. Details: [Authentication](./references/authentication.md).
