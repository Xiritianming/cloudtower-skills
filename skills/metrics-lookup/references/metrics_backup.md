...
| repo_probe_results_total | 存储库探测累计结果 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repository`、`result` |
| repo_info | 本 metric 通过 label 提供了仓库信息，用于渲染告警信息 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo_name`、`repository` |
| repo_stat_used_size_bytes | 仓库已用大小（以字节为单位） |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repository` |
| repo_stat_total_size_bytes | 仓库总大小（以字节为单位） |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repository` |
| file_read_time_nanoseconds_total | 按仓库 UUID 统计文件读取 I/O 所花费的时间 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo` |
| file_write_time_nanoseconds_total | 按仓库 UUID 统计的文件写入 IO 耗时 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo` |
| file_read_requests_total | 按仓库 UUID 处理的文件读取 I/O 请求 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo` |
| file_write_requests_total | 按仓库 UUID 处理的文件写入 I/O 请求 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo` |
| file_read_bytes_total | 按仓库 UUID 处理的文件读取 I/O 字节数 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo` |
| file_write_bytes_total | 按仓库 UUID 处理的文件写入 I/O 字节数 |  | `_tenant_id`、`service`、`service_id`、`instance`、`job`、`repo` |
