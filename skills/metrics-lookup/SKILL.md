...
**检索技巧**：关键词用中英双语的症状词（指标名是英文、说明是中文），如 error/错误、丢包/drop、延迟/latency。注意：脑中的具体术语可能两边都不出现——例如 `crc` 在目录中零命中，CRC 计数归并在 `host_network_receive_errors` 等 `*_errors` 指标里；无命中时改用上一层症状词重试，而不是断定指标不存在。
...
- 若接下来要实际查询这些指标的数据：查到的指标名可直接作为 `cloudtower-api` skill 中 Metrics 操作（`get-vm-metrics` 等）`metrics` 字段的取值——Tower 会代理观测服务，只需 CloudTower 凭证；详见该 skill 的 `references/metrics-guide.md`
